...
class LastComment extends Validator {
  ...
  async validate (context, validationSettings) {
    ...
    if (context.eventName === 'issue_comment' && context.payload.action === 'edited') {
      if (hasCommentChangedOption) {
        ...
        const commentThatChanged = this.getPayload(context, true).comment
      ...
      if (hasCommentChangedOption) {
        // with option not being evaluateable there is no comment to compare against
        // skip must_include and must_exclude and pass instead
        const result = {
          validator: validatorContext,
          status: 'pass',
          description: 'comment_changed_by and comment_changed_from are not evaluated since no edit occured',
          details: {
            input: 'nothing was edited',
            settings: validationSettings,
            error: undefined
          }
        }
        return consolidateResult([result], validatorContext)
      } else {
        // all the comments of the issue or pr
        comments = await this.githubAPI.listComments(context, issueNumber)
      }
  ...
  async filterByCommentChange (input, comments, commentChangedOption) {
    const filteredComments = Array.from(comments)
